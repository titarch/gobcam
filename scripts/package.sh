...
# Remove the staged sidecar on exit so a subsequent `tauri dev`
# (via `just ui-dev`) doesn't pick up the release binary as the
# externalBin source — Tauri 2 copies whatever sidecar it finds over
# the freshly-built debug binary at the workspace target/debug/ path,
# and a stale sidecar from a prior `just package` would silently
# regress dev to whatever CLI surface that release-build had.
WORK_DIR=""
cleanup() {
    rm -f "$SIDECAR_BIN"
