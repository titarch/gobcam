...
//! Workers skip files already on disk; progress is surfaced via
//! [`SyncProgress`].
...
/// Kick off the prefetch pool. Returns immediately.
pub(crate) fn spawn(
    catalog: &Arc<Catalog>,
    cache: &CacheRoot,
    ...
    // Finalizer flips `complete` once `fetched == total`.
...
fn worker_loop(
    ...
    loop {
        ...
        if let Err(e) = fetch_preview(entry, cache, downloader) {
            // Logged, not fatal: a synchronous retry happens on trigger.
