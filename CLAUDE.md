...
**Phase F** ✅ done. `scripts/gobcam-setup` is a single bash script that drops two snippets — `/etc/modules-load.d/gobcam.conf` (line: `v4l2loopback`) and `/etc/modprobe.d/gobcam.conf` (`options v4l2loopback devices=1 video_nr=10 card_label="Gobcam" exclusive_caps=1`) — so the kernel auto-loads the loopback at every boot with our options. Self-elevates via `pkexec` if not run as root; falls back to `sudo` instructions. Idempotent. `--uninstall` flag removes the snippets and `rmmod`s the module. Wired as `just install-loopback`. After running once, `just app` works unattended after every reboot — no manual `modprobe` step. README quickstart updated; the dev-only `scripts/sudoers-gobcam-dev` (passwordless `modprobe` for `just reset-loopback`) remains a separate, opt-in dev convenience.
