...
**Reproducible packaging via Docker** ✅ done. `just docker-package` (image: `docker/Dockerfile.package`, base `rust:1.92-slim-trixie`) bind-mounts the source tree into the container, runs the same `scripts/package.sh`, and produces .deb + AppImage in the host's `target/release/bundle/`. Trixie was chosen over Bookworm because Tauri 2 requires `libwebkit2gtk-4.1-dev` (only available there). Three runtime knobs the Dockerfile sets: `APPIMAGE_EXTRACT_AND_RUN=1` (linuxdeploy is itself an AppImage; without FUSE in unprivileged containers we need extract-and-run), `WEBKIT_DISABLE_DMABUF_RENDERER=1` (kept symmetric with the host recipe), and `NO_STRIP=true` (harmless on Trixie since its toolchain doesn't emit `.relr.dyn`, but kept for parity with `package.sh`). The container runs as `--user $(id -u):$(id -g)` so produced files are host-owned; `CARGO_HOME=/workspace/.cargo-docker` keeps the registry/git cache out of `~/.cargo`. Bundles built this way are ~5 MB .deb and ~115 MB AppImage (smaller than the Arch-native build because Trixie's bundled WebKit/GStreamer libs are slightly leaner). The native `just package` stays for fast iteration; `just docker-package` is the canonical "share-this-with-someone" path.
