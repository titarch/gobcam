...
6. **UI** ✅ done. `crates/ui/` is a Tauri 2 shell (Rust) + Svelte 5 frontend (TypeScript strict, Tailwind v4, Biome, Vitest, pnpm via corepack). The Rust shell holds a lazy `IpcClient` (single `UnixStream`, reconnects on failure) keyed off `--socket <path>` / `GOBCAM_SOCKET` / `$XDG_RUNTIME_DIR/gobcam.sock`. Tauri commands: `trigger`, `list_emoji`, `sync_status`. Daemon and UI run as two separate processes — UI re-uses the daemon's existing IPC surface, no daemonization needed yet. `just ui-dev` / `just ui-build`; `just check` gates both Rust and frontend. Biome lints `.ts` only because Biome 2.x doesn't yet model Svelte templates — `.svelte` files are covered by `svelte-check`.
