...
**Reproducible packaging via Docker** ✅ done. `just docker-package` (image: `docker/Dockerfile.package`, base `rust:1.92-slim-trixie`) bind-mounts the source tree into the container, runs the same `scripts/package.sh`, and produces .deb + AppImage in the host's `target/release/bundle/`. Trixie was chosen over Bookworm because Tauri 2 requires `libwebkit2gtk-4.1-dev` (only available there). Three runtime knobs the Dockerfile sets: `APPIMAGE_EXTRACT_AND_RUN=1` (linuxdeploy is itself an AppImage; without FUSE in unprivileged containers we need extract-and-run), `WEBKIT_DISABLE_DMABUF_RENDERER=1` (kept symmetric with the host recipe), and `NO_STRIP=true` (harmless on Trixie since its toolchain doesn't emit `.relr.dyn`, but kept for parity with `package.sh`). The container runs as `--user $(id -u):$(id -g)` so produced files are host-owned; `CARGO_HOME=/workspace/.cargo-docker` keeps the registry/git cache out of `~/.cargo`. A `--tmpfs` shadow on `crates/ui/node_modules` keeps the host's tree intact (pnpm hoists symlinks into `$HOME/store` which vanishes on container exit; without the shadow, the host's `pnpm install` sees a corrupted tree on the next `just check` and refuses to recover without a TTY). Bundles built this way are ~5 MB .deb and ~115 MB AppImage (smaller than the Arch-native build because Trixie's bundled WebKit/GStreamer libs are slightly leaner). The native `just package` stays for fast iteration; `just docker-package` is the canonical "share-this-with-someone" path.

**Packaging test harness** ✅ done. `just docker-test-deb` (= `scripts/test-deb.sh`) installs the produced .deb in a fresh `debian:trixie` container with `dpkg -i --force-depends` (skips dragging in WebKit/GStreamer/v4l2loopback-dkms — ~500 MB of deps unrelated to the maintainer-script logic), then asserts the `/etc/modules-load.d`, `/etc/modprobe.d`, and `/etc/sudoers.d/gobcam` snippets land correctly, the sudoers rule mentions `$SUDO_USER` and passes `visudo -c`, the `modprobe.d` options match exactly what the UI auto-reset path issues (drift detection — if either side ever changes, NOPASSWD won't authorise it), and `apt-get purge` removes everything cleanly. Containers can't exercise the kernel-module side (shared host kernel, no `CAP_SYS_MODULE`); for that, a real Debian/Ubuntu VM is needed. Critical implementation gotcha: `docker run` without `-i` closes stdin immediately, so a `bash <<'EOF'` heredoc reaches an empty stdin and silently outputs nothing — `-i` is required even though there's no interactive input. **`just uninstall-loopback`** wraps `bash scripts/gobcam-setup --uninstall` for testing the AppImage's first-run flow on a machine that already has Gobcam configured.

**AppImage in-app setup** ✅ done. AppImages can't drop `/etc/` files at install time, so the Phase G AppImage previously failed on first launch with "Element failed to change its state" (the daemon's `firewall::install` v4l2sink probe couldn't find `/dev/video10`). Fix: when `main.rs` detects the loopback device missing at startup, it skips the spawn and sets `DaemonSupervisor::setup_required = true` instead of erroring out. The frontend's `App.svelte` calls a new `setup_status` Tauri command on mount; if `required: true`, it renders `Setup.svelte` (button + explanation) instead of the main panel. Clicking the button calls `run_setup`, which execs the bundled `gobcam-setup` script (`crates/ui/src-tauri/src/setup.rs::locate` resolves it via `app.path().resource_dir()` for both .deb and AppImage layouts, with a workspace fallback for `tauri dev`); the script self-elevates via `pkexec` and writes the same three /etc files the .deb's postinst would have. On success, `run_setup` spawns the daemon and the UI flips to the main panel. Same flow works inside the .deb too — but there the postinst already ran, so `setup_required` is false on first launch.
