...
- **v4l2loopback can stick after a failed run**: if the daemon enters PLAYING but never pushes buffers (e.g. caps-negotiation deadlock), the loopback stays in OUTPUT-only mode and downstream `v4l2src` consumers see "not a capture device". Reset with `sudo rmmod v4l2loopback && just modprobe-loopback`.
...
Steps 1 (passthrough), 2 (always-on emoji overlay, static + animated), and 7 (Docker build env) done. Next: Step 3 — triggered overlay with timer (`stdin` line → splice/unlink subgraph cleanly).
